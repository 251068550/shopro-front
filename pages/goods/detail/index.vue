		...
		<view class="detail_box shopro-selector" v-else>
			<view class="detail-content">
				...
				<view class="title-box">
					...
					<view class="right" v-if="goodsInfo.activity && goodsInfo.activity.type === 'groupon'">
						<view>红包补贴</view>
						<view><text class="small">￥</text>{{goodsInfo.activity.rules.award}}</view>
					</view>
				</view>				
				<!-- 规格选择 -->
				<view
				...
				></shopro-sku>		
				<!-- 拼团规则 -->
				<sh-groupon-rule v-if="goodsInfo.activity && goodsInfo.activity.type === 'groupon'" :rules="goodsInfo.activity.rules"></sh-groupon-rule>	
					...
					<view class="detail-btn-box x-ac" v-if="goodsInfo.activity && goodsInfo.activity.type === 'groupon'">
						...
						<view class="x-f" v-if="activityRules.status == 'ing'">
							...
							<button class="cu-btn tool-btn groupon-btn y-f" :style="goodsInfo.activity.rules.is_alone === '0' ? 'width:400rpx' : ''" @tap="payGroupon('groupon')">
								<text class="price">￥{{ goodsInfo.groupon_price }}（定金￥{{ goodsInfo.deposit_price }}）</text>
...
import shGrouponRule from './children/sh-groupon-rule.vue';
