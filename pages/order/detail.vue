	...
	<view class="page_box">
		...
		<view class="content_box">
			...
			<view class="order-price-box" v-if="orderDetail.id">
				...
				<view class="notice-item x-bc" v-if="orderDetail.activity_type == 'groupon'">
					<text class="title">定金</text>
					<text class="detail">￥{{ orderDetail.deposit_price }}</text>
				</view>
				<view class="notice-item x-bc" v-if="orderDetail.activity_type == 'groupon'">
					<text class="title">尾款</text>
					<text class="detail">￥{{ orderDetail.final_payment }}</text>
				</view>
				...
				<view class="notice-item all-rpice-item x-f" style="width: 100%;" v-if="orderDetail.activity_type == 'groupon' && orderDetail.status_code == 'refund_finish'">
					<text class="title">拼团奖励：</text>
					<text class="detail all-price">￥+{{ orderDetail.log.award }}</text>
				</view>
			</view>
			<!-- 订单信息 -->
		...
		<view class="foot_box">
			...
			<view class="btn-box x-f">
				<view class="" v-for="btn in orderDetail.btns" :key="btn">
					<button v-if="btn === 'cancel' && orderDetail.activity_type == 'groupon' && orderDetail.group_step == 0" @tap.stop="onCancel(orderDetail.id)" class="cu-btn obtn1">取消订单</button>
					<button v-if="btn === 'cancel' && orderDetail.activity_type != 'groupon'" @tap.stop="onCancel(orderDetail.id)" class="cu-btn obtn1">取消订单</button>
