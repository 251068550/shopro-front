<template>
	<view class="page_box">
		...
		<view class="content_box">
			...
			<view class=" x-bc item-list border-top" v-if="grouponBuyType == 'groupon'">
				<view class="item-title">定金</view>
				<view class="x-f">
					<text class="price">￥{{ orderPre.deposit_price || '0.00' }}</text>
				</view>
			</view>
			<view class=" x-bc item-list border-top" v-if="grouponBuyType == 'groupon'">
				<view class="item-title">尾款</view>
				<view class="x-f">
					<text class="price">￥{{ orderPre.goods_amount - orderPre.deposit_price || '0.00' }}</text>
				</view>
			</view>
		</view>
		<view class="foot_box x-f">
			<text class="num">共1件</text>
			<view class="all-money" v-if="grouponBuyType == 'groupon'">
				<text>定金：</text>
				<text class="price">￥{{ orderPre.deposit_price || '0.00' }}</text>
			</view>		
