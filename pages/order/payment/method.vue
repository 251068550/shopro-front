	...
	<view class="page_box">
		...
		<view class="content_box">
			<view class="y-f money-box" v-if="orderDetail.total_fee">
				...
				<view class="money" v-if="orderDetail.activity_type == 'groupon' && orderDetail.group_step == 0">{{ orderDetail.deposit_price }}</view>
				<view class="money" v-else-if="orderDetail.activity_type == 'groupon' && orderDetail.group_step == 1">{{ orderDetail.final_payment }}</view>
				<view class="money" v-else>{{ orderDetail.total_fee }}</view>
			</view>
			...
			<view class="group_tips" v-if="orderDetail.activity_type == 'groupon' && orderDetail.group_step == 1">注意：请在支付剩余时间内支付尾款，如果超时未支付尾款，定金恕不退回，同时订单作废失效。</view>
			<view class="x-c">
				<button class="cu-btn pay-btn" @tap="confirmPay" v-if="orderDetail.activity_type == 'groupon' && orderDetail.group_step == 0">支付定金 ￥{{ orderDetail.deposit_price }}</button>
				<button class="cu-btn pay-btn" @tap="confirmPay" v-else-if="orderDetail.activity_type == 'groupon' && orderDetail.group_step == 1">支付尾款 ￥{{ orderDetail.final_payment }}</button>
				<button class="cu-btn pay-btn" @tap="confirmPay" v-else>确认支付 ￥{{ orderDetail.total_fee }}</button>
			</view>
		</view>
