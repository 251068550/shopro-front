		...
		<view class="content_box">
			<scroll-view scroll-y="true" enable-back-to-top @scrolltolower="loadMore" class="scroll-box">
				<view class="order-list" v-for="(order, orderIndex) in orderList" :key="order.id" @tap.stop="jump('/pages/order/detail', { id: order.id })">
					...
					<view class="order-groupon x-bc" v-if="order.activity_type == 'groupon' && order.status_code == 'refund_finish'">
					...
					<view class="order-bottom">
						...
						<view class="btn-box x-f" v-if="order.btns.length">
							<block v-for="orderBtn in order.btns" :key="orderBtn">
								<button v-if="orderBtn === 'cancel' && order.activity_type == 'groupon' && order.group_step == 0" @tap.stop="onCancel(order.id, orderIndex)" class="cu-btn obtn1">取消订单</button>
								<button v-if="orderBtn === 'cancel' && order.activity_type != 'groupon'" @tap.stop="onCancel(order.id, orderIndex)" class="cu-btn obtn1">取消订单</button>
